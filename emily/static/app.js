...
function renderGrades(grades) {
  ...
  const rows = grades
    .map(
      (grade) => `
        <tr>
          <td>${escapeHtml(grade.first_name)} ${escapeHtml(grade.last_name)}</td>
...
submitForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(result.error || "Unable to submit grade");
    }
...
viewForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const result = await response.json();
    if (!response.ok) {
      throw new Error(result.error || "Unable to fetch grades");
    }
